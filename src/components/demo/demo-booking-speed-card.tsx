...
export function DemoBookingSpeedCard({
  ...
  return (
    <section
      className="mt-5 overflow-hidden rounded-3xl border px-4 py-3.5 text-center shadow-[0_18px_40px_-24px_rgba(94,200,255,0.85)]"
      style={{
    ...
    >
      <p
        className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em]"
        style={{ color: DEMO_SPEED_AZZURRO }}
      >
        <Zap className="h-3 w-3" strokeWidth={2.75} aria-hidden />
        Prenotazione lampo
      </p>

      <div
        className="mt-2 flex items-end justify-center gap-2.5 tabular-nums"
        aria-label={duration.label}
      >
        <TimeBlock value={pad2(major)} unit={majorUnit} />
        <span
          className="mb-3.5 text-2xl font-black"
          style={{ color: DEMO_SPEED_AZZURRO }}
          aria-hidden
        >
          :
      ...
      <p className="mt-3 text-sm font-bold leading-snug text-primary-black">
      ...
      <p className="mt-1 text-xs leading-snug text-primary-black/62">
...
function TimeBlock({ value, unit }: { value: string; unit: string }) {
  return (
    <span className="flex min-w-[3.75rem] flex-col items-center">
      <span
        className="text-[2.15rem] font-black leading-none tracking-tight"
        style={{
          color: DEMO_SPEED_AZZURRO,
          textShadow: "0 0 18px rgba(94, 200, 255, 0.45)",
        }}
      >
        {value}
      </span>
      <span className="mt-1 text-[9px] font-black uppercase tracking-[0.2em] text-primary-black/45">
        {unit}
      </span>
    </span>
  );
