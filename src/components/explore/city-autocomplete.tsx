      ...
      <div className="relative">
        ...
        {value && (
          <button
            ...
            className="absolute right-3 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full bg-ink-inverse/5 text-ink-inverse/50"
          ...
          {suggestions.map((city) => (
            <li key={city}>
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-ink-inverse transition-colors hover:bg-brand-teal/8",
                  value === city && "bg-brand-teal/10 font-medium text-brand-teal",
                )}
              >
                <MapPin className="h-3.5 w-3.5 shrink-0 text-ink-inverse/40" />
                {city}
              </button>
            </li>
          ))}
